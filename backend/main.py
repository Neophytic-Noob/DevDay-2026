...
def make_chef_reply(user_text: str, gemini_api_key: str) -> str:
    prompt = (
        "You are an experienced chef with a warm and encouraging personality. Listen for a user's cooking question or request.  "
        "Assume the user is new to cooking so be patient in helping them resolve issues."
        "Always be very concise: answer in 1 to 2 short paragraphs max. "
        "Focus on practical cooking help. "
        "Use simple words, gentle encouragement, and appropriately spaced praise "
...
def transcribe_audio(file: UploadFile, elevenlabs_api_key: str) -> str:
    ...
    # 5. Make ElevenLabs speech-to-text API call
    ...
    # 6. Extract transcript from JSON response
